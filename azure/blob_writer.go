...
func NewAzureBlobRotateWriter(storageAccount, storageAccessKey, containerName, objectName string, rotationSize int, endpoint string) (*BlobRotateWriter, error) {
    return newAzureBlobRotateWriter(storageAccount, storageAccessKey, containerName, objectName, endpoint, rotationSize)
}

func newAzureBlobRotateWriter(storageAccount, storageAccessKey, containerName, objectName, endpoint string, rotationSize int) (w *BlobRotateWriter, err error) {
    w = &BlobRotateWriter{
...
func (w *BlobRotateWriter) Write(output []byte) (int, error) {
    w.lock.Lock()
    defer w.lock.Unlock()
...
func (w *BlobRotateWriter) Close() (err error) {
...
func (w *BlobRotateWriter) Rotate() (err error) {
    if w.pipeWriter != nil {
...
func (w *BlobRotateWriter) stream() (err error) {
    waitGroup.Add(1)
